...
def test_judge_available_true_with_real_key(monkeypatch):
    monkeypatch.delenv("ATOMIC_ATLAS_NO_ATTACKER_LLM", raising=False)
...
def test_judge_available_false_with_no_attacker_llm_flag(monkeypatch):
    monkeypatch.setenv("OPENAI_API_KEY", "sk-real-1234")
    monkeypatch.setenv("ATOMIC_ATLAS_NO_ATTACKER_LLM", "1")
    assert _judge_available() is False
...
def test_auto_picks_indicators_without_judge(monkeypatch):
    ...
    monkeypatch.delenv("OPENAI_API_KEY", raising=False)
    monkeypatch.delenv("ATOMIC_ATLAS_NO_ATTACKER_LLM", raising=False)
    monkeypatch.delenv("ATOMIC_ATLAS_SCORING", raising=False)
...
def test_auto_picks_substring_when_no_indicators(monkeypatch):
    monkeypatch.delenv("OPENAI_API_KEY", raising=False)
    monkeypatch.delenv("ATOMIC_ATLAS_NO_ATTACKER_LLM", raising=False)
    monkeypatch.delenv("ATOMIC_ATLAS_SCORING", raising=False)
    ...
    scorer = _select_scorer(atomic)
    primary = getattr(scorer, "_primary", scorer)
    assert primary.__class__.__name__ == "SubStringScorer"
...
def test_explicit_indicators_without_indicators_raises(monkeypatch):
    monkeypatch.delenv("OPENAI_API_KEY", raising=False)
    atomic = _atomic(success_indicators=None, scoring={"strategy": "indicators"})
    with pytest.raises(ValueError, match="success_indicators"):
        _select_scorer(atomic)


def test_explicit_substring_strategy(monkeypatch):
    monkeypatch.delenv("OPENAI_API_KEY", raising=False)
    atomic = _atomic(scoring={"strategy": "substring"})
    scorer = _select_scorer(atomic)
    primary = getattr(scorer, "_primary", scorer)
    assert primary.__class__.__name__ == "SubStringScorer"
...
def test_env_override_scoring_strategy(monkeypatch):
    monkeypatch.setenv("OPENAI_API_KEY", "sk-real-1234")
    monkeypatch.setenv("ATOMIC_ATLAS_SCORING", "indicators")
    atomic = _atomic(success_indicators=["pwned"])  # judge would've been auto
    scorer = _select_scorer(atomic)
    primary = getattr(scorer, "_primary", scorer)
    assert primary.__class__.__name__ == "_IndicatorScorer"


def test_refusal_off_skips_wrapping(monkeypatch):
    monkeypatch.delenv("OPENAI_API_KEY", raising=False)
    atomic = _atomic(
        success_indicators=["pwned"],
        scoring={"strategy": "indicators", "refusal_check": "off"},
    )
    scorer = _select_scorer(atomic)
    # When refusal_check=off, build_refusal_short_circuit returns the primary
